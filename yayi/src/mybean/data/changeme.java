	...
	public void setPageSize(int size){
		...
		try{
			...
			if(name.equals("all"))
				...
				rs = sql.executeQuery("select * from "+tableName+" where username='"+name+"' and 账号='"+username+"'");
	...
	public StringBuffer show(int page){
		...
		try{
			...
			for(int i = 1;i<=pageSize&&boo;i++){
				...
				str.append("<td><form action=\"/yayi/d\" method=\"post\"><input type=hidden name=\"jieshou\" value=\""+rs.getString(1)+","+rs.getString(5)+","+rs.getString(4)+"\""+"><input type=submit name=\"\" value=\"修改预约\" ><input type=hidden name=\"title\" value=\"changeme\"></form></td>");
				str.append("<td><form action=\"/yayi/delete1\" method=\"post\"><input type=hidden name=\"quxiao\" value=\""+rs.getString(1)+","+rs.getString(5)+","+rs.getString(4)+"\""+"><input type=submit name=\"\" value=\"取消预约\" onclick=\"delcfm()\"><input type=hidden name=\"title\" value=\"changeme\"></form></td>");				
				str.append("</tr>");
				boo = rs.next();
			}
		}catch(Exception e){}
