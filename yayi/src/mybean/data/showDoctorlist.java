	...
	public StringBuffer show(int page){
		...
		try{
			...
			for(int i = 1;i<=pageSize&&boo;i++){
				str.append("<tr>");
				for(int k = 1;k<zid+1;k++){
					str.append("<td>"+rs.getString(k)+"</td>");
				}
				//str.append("<td><form action=\"/yayi/jieshou1\"><input type=hidden name=\"jieshou\" value=\""+rs.getString(1)+","+rs.getString(2)+"\""+"><input type=submit name=\"\" value=\"管理员\" onclick=\"checkjs()\"><input type=hidden name=\"title\" value=\"userlist\" method=\"post\"></form></td>");
				str.append("<td><form action=\"/yayi/delete1\"  method=\"post\"><input type=hidden name=\"quxiao\" value=\""+rs.getString(1)+","+rs.getString(2)+"\""+"><input type=submit name=\"\" value=\"删除信息\" onclick=\"delcfm()\"><input type=hidden name=\"title\" value=\"doctorlist\"></form></td>");
				str.append("</tr>");
				boo = rs.next();
			}
		}catch(Exception e){}
